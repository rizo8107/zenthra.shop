...
var pb = new Client("https://backend-pocketbase.7za6uc.easypanel.host/");
var AUTH_TIMEOUT = 1e4;
var MAX_RETRIES = 3;
var initAdmin = async (retryCount = 0) => {
  try {
    ...
    if (error?.status === 0 && retryCount < MAX_RETRIES) {
      console.log(`Retrying authentication in ${(retryCount + 1) * 1e3}ms...`);
      await new Promise((resolve) => setTimeout(resolve, (retryCount + 1) * 1e3));
...
var authenticateAdmin = async (email, password) => {
  try {
...
var getDashboardMetrics = async () => {
  try {
    ...
    const revenueToday = ordersResult.filter((order) => {
...
var getMonthlyRevenueData = async () => {
  try {
    await ensureAdminAuth();
    const ordersResult = await pb.collection("orders").getFullList({
      sort: "created"
...
var getImageUrl = (collectionId, recordId, fileName) => {
  ...
  return `${"https://backend-pocketbase.7za6uc.easypanel.host/"}/api/files/${collectionId}/${recordId}/${fileName}`;
};
export {
  authenticateAdmin,
  ensureAdminAuth,
