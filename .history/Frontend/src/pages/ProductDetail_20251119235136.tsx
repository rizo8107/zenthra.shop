...
const ProductDetail = () => {
  ...
  const { isInCart, currentItemQty, currentItemUnitPrice } = useMemo(() => {
    ...
    return {
      ...
      currentItemUnitPrice: match
        ? typeof match.unitPrice === 'number'
          ? match.unitPrice
          : Number(match.product?.price) || 0
        : 0,
    };
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      try {
        ...
        const initSizesRaw = (data as any).variants?.sizes || [];
        const initSizes = Array.isArray(initSizesRaw)
          ? initSizesRaw.map((size: any) => normalizeSizeVariant(size))
          : [];
  ...
  const stickyCartTotal = currentItemQty > 0 ? currentItemUnitPrice * currentItemQty : selectedTotal;

  const resolvedOriginalPrice = useMemo(() => {
    if (!product) return undefined;
    const variantOriginal = selectedSize && typeof selectedSize.originalPrice === 'number'
      ? selectedSize.originalPrice
      : undefined;
    const productOriginal = typeof product.original_price === 'number' ? product.original_price : undefined;
  ...
  const standardCombos = comboOptions.filter((cb: any) => (cb?.type || '').toLowerCase() !== 'bundle');
  const bundleCombos = comboOptions.filter((cb: any) => (cb?.type || '').toLowerCase() === 'bundle');
  const colorOptions =
        ...
        <div className="grid gap-6 md:gap-10 md:grid-cols-2 md:pt-4">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sizeOptions.map((sz: any) => {
                      ...
                      const finalPrice = Number.isFinite(priceInfo.final) ? priceInfo.final : 0;
                      const originalPrice = priceInfo.original && priceInfo.original > finalPrice ? priceInfo.original : undefined;
                      return (
                        <button
                          key={sz.value}
                          type="button"
              ...
              {/* Buy Any X Combos - Mix & Match bundles driven by backend data */}
              {bundleCombos.length > 0 && (
                <div className="space-y-4">
                  ...
                  {/* Show variant selection when a bundle combo is selected */}
                  {selectedCombo && bundleCombos.some((b: any) => b.value === selectedCombo.value) && (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-foreground">
                          {selectedCombo.name || 'Bundle Selection'} 
                          <span className="text-xs font-normal text-muted-foreground ml-1">
                            (Select {selectedCombo.requiredQuantity || selectedCombo.items || 2} variants)
                          </span>
                        </span>
                        ...
                        {(selectedBuyAnyXProducts[selectedCombo.value] || []).length === 0 && (
                          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            ...
                            <div className="text-xs text-yellow-700">
                              Select {selectedCombo.requiredQuantity || selectedCombo.items || 2} {selectedCombo.allowDuplicates === false ? 'different ' : ''}variants below to create your personalized bundle!
                            </div>
                          </div>
                        )}
                        <div className="space-y-4">
                        ...
                        <div className="mt-4 p-3 bg-blue-100/50 rounded-lg">
                          ...
                          <div className="text-xs text-blue-700">
                            ...
                            • {selectedCombo.discountType === 'percent'
                                ...
                                : selectedCombo.discountType === 'amount'
                                  ? `₹${selectedCombo.discountValue} off`
