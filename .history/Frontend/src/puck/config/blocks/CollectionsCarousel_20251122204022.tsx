  ...
  if (mode !== "carousel") {
    return (
      ...
      >
        ...
        <div className={cn("grid", `grid-cols-${Math.max(1, mobileCount)}`, `sm:grid-cols-${Math.max(1, tabletCount)}`, `lg:grid-cols-${Math.max(1, desktopCount)}`)} style={{ gap: `${itemGap}px` }}>
          {data.map((it, i) => (
            <Item key={i} {...it} />)
          )}
        </div>
          ...
          <div className="flex transition-transform duration-300 will-change-transform" style={{ transform: `translate3d(-${page * 100}%, 0, 0)` }}>
            {pages.map((group, gi) => (
              <div key={gi} className="shrink-0 w-full">
                <div className={cn("grid", `grid-cols-${perRow}`)} style={{ gap: `${itemGap}px` }}>
                  {group.map((it, i) => (
                    <Item key={`${gi}-${i}`} {...it} />
                  ))}
                </div>
