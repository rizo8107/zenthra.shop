...
const ProductCard = ({ product, priority = false, overrides }: ProductCardProps) => {
  ...
  return (
    ...
    >
      <div className={cn('relative overflow-hidden bg-muted', aspectCls)}>
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
          {product.bestseller && (
            <Badge
              variant="secondary"
              className="bg-foreground/80 text-background backdrop-blur-sm rounded-full px-3 py-1 text-xs font-medium"
            >
              Best Seller
            </Badge>
          )}
          {product.new && (
            <Badge
              variant="secondary"
              className="bg-primary/80 backdrop-blur-sm text-primary-foreground rounded-full px-3 py-1 text-xs font-medium"
            >
              New
            </Badge>
          )}
          {pc.showTags &&
            ...
            product.tags.slice(0, 3).map((t) => (
              <span
                key={String(t)}
                className="px-2 py-0.5 rounded-full bg-background/90 text-foreground text-[10px] shadow-sm"
              >
                {String(t)}
              </span>
            ))}
      ...
      <div className={cn(bodyPadding, 'flex flex-col gap-2')}>
        <div>
          <h3
            className={cn(
