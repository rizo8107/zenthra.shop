// Frontend Checkout Flow Helper
// Mirrors the backend checkout flow logic in a lightweight way so
// the storefront can decide which payment methods are allowed
...
export interface CheckoutCondition {
  ...
  amount?: number;      // for min/max total
  state?: string;       // for state_* conditions
  country?: string;     // for country_is
  value?: boolean;      // for user_logged_in / tn_shipping_restricted
}
...
export interface CheckoutItemContext {
  ...
  category?: string;
  tags?: string[];
  tn_shipping_enabled?: boolean; // false means restricted for Tamil Nadu
}
