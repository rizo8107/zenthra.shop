import { pb as pocketbase } from './pocketbase';
...
export function getContentImageUrl(record: ContentItem, filename?: string | string[]): string {
  ...
  if (!file) return '';
  return pocketbase.files.getURL(record as any, file as string);
}
...
export function getOptimizedContentImageUrl(
  ...
  const baseUrl = pocketbase.files.getURL(record as any, first as string);
