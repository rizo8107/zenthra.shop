/**
 * Webhook Server API Client (Backend)
 * 
 * This service handles all communication with the standalone webhook/payment server from the backend.
 */
...
/**
 * Manage webhook subscriptions (requires admin API key)
 */
export async function manageWebhookSubscription(
  action: 'list' | 'create' | 'update' | 'delete',
  data?: any,
  id?: string
) {
  const adminApiKey = import.meta.env.VITE_WEBHOOKS_ADMIN_API_KEY;
  
  if (!adminApiKey) {
    throw new Error('WEBHOOKS_ADMIN_API_KEY not configured');
  }

  let url = `${WEBHOOK_SERVER_URL}/api/webhooks/subscriptions`;
  let method = 'GET';
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Webhook subscription operation failed');
    }
