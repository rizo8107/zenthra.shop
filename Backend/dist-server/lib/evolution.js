...
const ENV_SERVER_PORT = Number(import.meta?.env?.VITE_SERVER_PORT) || 3001;
const CANDIDATE_PORTS = [ENV_SERVER_PORT, 3002, 3003, 4001, 4002, 4003];
let RESOLVED_SERVER_URL = ENV_SERVER_URL || null;
const fetchWithTimeout = async (url, ms = 1200) => {
...
export const sendWhatsAppMediaMessage = async (data) => {
    try {
        // Format the phone number to ensure it has 91 country code
        const formattedPhone = formatPhoneNumber(data.phone);
        const requestData = {
...
export const getInstanceConnectionState = async (instanceName) => {
    try {
