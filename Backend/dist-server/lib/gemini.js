 ...
 */
export async function generateProductCopy(request) {
 ...
 */
export async function generatePuckContent(request) {
    const tone = request.tone || 'playful';
...
`
    ...
    const existingTypes = Array.isArray(request.existingTypes)
        ? request.existingTypes.filter((v) => typeof v === 'string' && v.trim().length > 0)
        : [];
...
Already-placed component types you should avoid reusing unless absolutely necessary:
${existingTypes.length ? existingTypes.map((t) => ` - ${t}`).join('\n') : ' - (none)'}

General rules:
- Use realistic but concise marketing copy.
- Use short headings and 1–3 line body text.
- Do NOT include markdown, backticks, code fences, or comments.
- Do NOT explain what you are doing.
 - Aim for fresh layouts that differ from existing sections.
`;
...
- This is a list of 1–4 blocks to insert below the current selection.
...
`;
    ...
    try {
        ...
        const maybeInjectProductDetailBlock = (blocks) => {
            if (!product)
                return blocks;
            const hasProductBlock = blocks.some((b) => typeof b.type === 'string' && b.type.toLowerCase().includes('product'));
            if (hasProductBlock)
                return blocks;
        ...
        if (request.mode === 'page') {
            ...
            return {
                root: page.root && typeof page.root === 'object' && !Array.isArray(page.root)
                    ? page.root
                    : {},
                content: injected,
            };
        ...
        if (parsed && Array.isArray(parsed.content)) {
            const normalised = parsed.content
                .map(normaliseBlock)
                .filter((b) => Boolean(b));
            return maybeInjectProductDetailBlock(normalised);
        }
