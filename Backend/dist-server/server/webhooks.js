...
import { createWebhook, deleteWebhook, listWebhooks, updateWebhook } from './webhookStore.js';
import { emitEvent } from './webhookDispatcher.js';
...
router.post('/emit', async (req, res) => {
    try {
        const body = req.body || {};
        const event = {
            id: body.id || crypto.randomUUID?.() || 'evt_' + Math.random().toString(36).slice(2, 10),
            type: body.type,
        ...
        const targets = Array.isArray(body.targets) ? body.targets.filter((u) => typeof u === 'string' && u.trim()) : [];
        if (targets.length > 0) {
            const tempSubs = targets.map((url) => ({ url, events: [event.type], active: true, retries: 0, timeout_ms: 8000 }));
            await emitEvent(event, tempSubs);
        }
        else {
