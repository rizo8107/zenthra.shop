...
import { listWebhooks, recordWebhookFailure } from './webhookStore.js';
function signPayload(secret, raw) {
...
export async function emitEvent(event, subscriptions) {
    try {
        ...
        await Promise.all(matching.map((sub) => dispatchToSubscription(sub, event).catch((err) => {
